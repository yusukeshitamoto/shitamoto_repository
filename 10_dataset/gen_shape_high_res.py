...
obj_path = os.path.join(dirname, "data_shape_high")
try:
    os.mkdir(obj_path)
except:
...
if __name__ == "__main__":
    ...
    # 円を保存
    img = fnd_shapes.generate_circle(L, 40 * m, center=[L/2+0.5, L/2+0.5])
    save_shape.save_shape(obj_path, img, "01")


    # 左むき三角形を保存
    img = fnd_shapes.generate_triangle_at_center(L, 100 * m)
    img = np.rot90(img)
    ...
    # 正方形を保存
    img = fnd_shapes.generate_square_at_center(L, 80 * m)
