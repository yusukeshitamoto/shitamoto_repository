...
class ObjFunc():
    ...
    def save_log(self):
        ...
        plt.xlabel("Step")
        plt.ylabel("$J$")
        plt.savefig(os.path.join(self.dir_main, "J_log.png"))

    def save_opt_shape(self, x):
        input = torch.Tensor(x)
        img = self.vae.decoder(input)
        img = img.detach().numpy().reshape(128, 128)*-1
        img = (img > 0.) * 1.
    ...
    def save_log_as_csv(self):
        obj_path = os.path.join(self.dir_main, "J_log.csv")
        df = pd.DataFrame(self.J_log)
        df.to_csv(obj_path, header=False, index=False)
        obj_path_z = os.path.join(self.dir_main, "z_log.csv")
    ...
    def export_iteration(self):
        ...
        with open(os.path.join(self.dir_main, file), "w") as f:
            print(self.iteration, file=f)

    def preprocess_for_bayes(self, x):
        """bayes最適化はsqueezeしないといけない．．．"""
        return x.squeeze()
